 ...
 */
const Notification: FC<{ message: string; onClose: () => void; isError?: boolean }> = ({ message, onClose, isError }) => {
  if (!message) return null;

  return (
    <div className={`fixed top-5 right-5 p-4 rounded-md shadow-lg text-white ${isError ? 'bg-red-500' : 'bg-blue-500'}`}>
      <span>{message}</span>
...
const AuthPage: FC = () => {
  ...
  const handleLogin = async (e: MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      showNotification("An error occurred while logging in. Please check your login details.", true);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSignup = async (e: MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      showNotification("An error occurred while signing up. Please check your details and try again.", true);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      showNotification("An error occurred while logging out. Please try again.", true);
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="w-full h-screen flex flex-col items-center justify-center p-4">
        ...
        <div className="w-full max-w-md p-8 space-y-6 text-center">
            ...
            <p className="text-lg font-medium text-blue-400 break-all">{session.user.email}</p>
            <button 
    ...
    <div className="w-full h-screen flex items-center justify-center p-4">
        ...
        <div className="w-full max-w-md p-8 space-y-6">
            <header>
                ...
                <p className="text-center text-gray-500 mt-2">Sign in or create an account.</p>
            </header>
